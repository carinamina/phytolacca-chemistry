#started 4-23-18 during jury duty ... continued by gri in 2020
...
source("B2_toughness_wrangling.R")
...
#chem <- read.csv("data_out/20180531_chem_abundant1percent.csv", header = TRUE)
chem$X <- NULL
str(chem[,1:14])
...
write.table(colnames(all[,8:11]), "data_out/nochem_list.txt", sep="\t", row.names = FALSE, quote=FALSE, col.names=FALSE)
#chemsum: shann, richness, abundance
write.table(colnames(all[,12:14]), "data_out/chemsum_list.txt", sep="\t", row.names = FALSE, quote=FALSE, col.names=FALSE)
#chemical compounds
write.table(colnames(all[,15:ncol(all)]), "data_out/chem_list.txt", sep="\t", row.names = FALSE, quote=FALSE, col.names=FALSE)
#export line_age and pop to turn into a binary matrix
write.table(all[,1:2], "data_out/all_pop", sep="\t", row.names = FALSE, quote=FALSE)
#pop_binary <- read.csv("all_pop_binary.matrix.txt", sep = "\t", header=TRUE) 
pop_binary <- read.csv("data_out/all_pop", sep = "\t", header=TRUE) 
#write the population binary list
write.table(colnames(pop_binary[,2:ncol(pop_binary)]), "data_out/pop_list.txt", sep="\t", row.names = FALSE, quote=FALSE, col.names=FALSE)
...
write.table(mature, "data_out/mature_20180507", sep="\t", row.names = FALSE)

young <- subset(all, all$age == "young")
young <- young[c(1,7:ncol(young))]
write.table(young, "data_out/young_20180507", sep="\t", row.names = FALSE)
